...
ErrorStatus CNT_RTC_SetAlarmATime(uint8_t hours, uint8_t minutes, uint8_t seconds)  // настройка просыпаться в заданное время ежедневно
{
  ...
  exticonf.EXTI_Line = EXTI_Line17; // конфиг 17-й линии прерывания
  exticonf.EXTI_Mode = EXTI_Mode_Interrupt;
  exticonf.EXTI_Trigger = EXTI_Trigger_Rising;
  exticonf.EXTI_LineCmd = ENABLE;
  EXTI_Init(&exticonf); 
...
void CNT_RTC_ClearWakeupFlags(void) // сброс флагов прерывания по париодическому таймеру
{
  RTC_ClearFlag(RTC_FLAG_WUTF);
  RTC_ClearITPendingBit(RTC_IT_WUT);
...
void CNT_RTC_ClearAlarmAFlags(void) // сборос флагов аларм А - будильник
{
